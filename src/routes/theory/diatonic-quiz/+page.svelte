<script>
  ...
  function diatonicChord(rootIdx, degree, scaleType, use7ths) {
    const iv = scaleType === 'major' ? MAJOR_IV : MINOR_IV;
    const note = NOTES[(rootIdx + iv[degree]) % 12];
    let q;
    if (use7ths) {
      q = scaleType === 'major' ? MAJ_7TH_Q[degree] : MIN_7TH_Q[degree];
    } else {
      q = scaleType === 'major' ? MAJ_TRIAD_Q[degree] : MIN_TRIAD_Q[degree];
    }
    return note + q;
  }

  function romanNumeral(degree, scaleType) {
    return scaleType === 'major' ? MAJ_ROM[degree] : MIN_ROM[degree];
  }
  ...
  function genQ() {
    ...
    if (mode === 'd2c') {
      ...
      // Build distractors from other degrees in the same key
      const pool = [];
      for (let i = 0; i < 7; i++) {
        if (i === degree) continue;
        pool.push(diatonicChord(rootIdx, i, scaleType, d.use7ths));
      }
      dqShuffle(pool);
      const unique = [correctChord];
      for (const c of pool) { if (!unique.includes(c) && unique.length < 4) unique.push(c); }
      // Fill with random notes if needed
      while (unique.length < 4) {
        const rr = NOTES[Math.floor(Math.random() * 12)];
        const rq = d.use7ths ? MAJ_7TH_Q[Math.floor(Math.random() * 7)] : MAJ_TRIAD_Q[Math.floor(Math.random() * 7)];
        const name = rr + rq;
        if (!unique.includes(name)) unique.push(name);
      }
      const ch = dqShuffle(unique.slice(0, 4));
      return {
        prompt: `<div class="qz-prompt-sub">What is the ${rom} chord in ${rootNote} ${scaleLabel(scaleType)}?</div>`,
        choices: ch,
        correctIdx: ch.indexOf(correctChord),
        extra: ''
      };
    } else {
      // "What Roman numeral is Am in C major?" -> answer is a Roman numeral
      const correctChord = diatonicChord(rootIdx, degree, scaleType, d.use7ths);
      const correctRom = romanNumeral(degree, scaleType);

      // Build distractors from other numerals in the same key
      const pool = [];
      for (let i = 0; i < 7; i++) {
        if (i === degree) continue;
        pool.push(romanNumeral(i, scaleType));
      }
      dqShuffle(pool);
      const unique = [correctRom];
      for (const r of pool) { if (!unique.includes(r) && unique.length < 4) unique.push(r); }
      const ch = dqShuffle(unique.slice(0, 4));
      return {
        prompt: `<div class="qz-prompt-sub">What Roman numeral is ${correctChord} in ${rootNote} ${scaleLabel(scaleType)}?</div>`,
        choices: ch,
        correctIdx: ch.indexOf(correctRom),
        extra: ''
      };
    }
  }
...
<svelte:head>
  <title>Diatonic Quiz — Guitar Theory Exercise</title>
  <meta name="description" content="Test your diatonic chord knowledge — identify chords by Roman numeral and degree in any key.">
</svelte:head>
